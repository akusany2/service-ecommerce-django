$(document).ready(function () {
  ...
  $("#clientPublic [name='service-item']").on("change", function (e) {
    if ($(e.target).is(":checked")) {
      totalPrice += $(e.target).parents('.client-selection').find('.price').data('price')
    } else {
      totalPrice -= $(e.target).parents('.client-selection').find('.price').data('price')
    }
    $buyBtn.html("Pay €" + totalPrice)
    if (totalPrice == 0) {
  ...
  function bounce(node) {
    ...
    node[1] = (waveHeight / 2) * Math.sin(node[2] / 20) + height / 1.2;
